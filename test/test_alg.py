...
class TestAlg(unittest.TestCase):
    ...
    def test_data_samples(self):
        ...
        self.assertEqual(bruteforce([-.121, .121, 5.4, 9, 6], 0,
                                    self.evalexpress), [(-.121, .121),
                                    (.121, -.121)])
        self.assertEqual(dictalg([-.121, .121, 5.4, 9, 6], 0,
                                    self.evalexpress), [(-.121, .121),
                                    (.121, -.121)])
        ...
        self.assertEqual(bruteforce([4, 1.0001, 1, 2, 4, 0], 2.0000,
                        ...
                        (1, 1), (2, 0), (0, 2)])
        self.assertEqual(dictalg([4, 1.0001, 1, 2, 4, 0], 2.0000,
                        self.evalexpress), [(1.0001, 1), (1, 1.0001),
                        (1, 1), (2, 0), (0, 2)])

    def test_failure(self):
        """
            the second unittest causes the method, bruteForce, to fail if given wrong input such as a character when only floats are allowed.
